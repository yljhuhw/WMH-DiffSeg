...
class TrainLoop:
    ...
    def save(self):
        ...
        if dist.get_rank() == 0:
            ...
            # Keep only the latest checkpoint of each type
            import glob as _glob
            logdir = get_blob_logdir()
            for pattern in ["savedmodel*.pt", "optsavedmodel*.pt"]:
                files = sorted(_glob.glob(os.path.join(logdir, pattern)))
                for f in files[:-1]:
                    os.remove(f)
            for rate in self.ema_rate:
                ema_files = sorted(_glob.glob(os.path.join(logdir, f"emasavedmodel_{rate}_*.pt")))
                for f in ema_files[:-1]:
                    os.remove(f)
