...
class ModelMeanType(enum.Enum):
    ...
    V_PREDICTION = enum.auto()  # v = alpha_t * eps - sigma_t * x_0
...
class GaussianDiffusion:
    ...
    def training_losses_segmentation_wmh(self, model, x_start, t, lambda_dice=0.1, lambda_cross_scale=0.05, model_kwargs=None, noise=None):
        """WMH two-stage v-prediction training losses."""
        from .wmh_losses import compute_v_target, dice_bce_loss, cross_scale_consistency_loss
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start[:, -1:, ...])
        ...
        mask = x_start[:, -1:, ...]
        res = torch.where(mask > 0, 1, 0).float()
        res_t = self.q_sample(res, t, noise=noise)

        x_t = x_start.float().clone()
        x_t[:, -1:, ...] = res_t

        sqrt_a = _extract_into_tensor(self.sqrt_alphas_cumprod, t, res.shape)
        sqrt_1a = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, res.shape)
        v_target = compute_v_target(res, noise, sqrt_a, sqrt_1a)

        # model forward: returns ((fine_out, cal), coarse_pred)
        model_out = model(x_t, self._scale_timesteps(t), **model_kwargs)
        if isinstance(model_out, tuple) and len(model_out) == 2:
            (fine_out, cal), coarse_pred = model_out
        else:
            fine_out, cal = model_out
            coarse_pred = None

        terms = {}
        terms["loss_v_fine"] = mean_flat((v_target - fine_out) ** 2)
        cal_resized = F.interpolate(cal, size=res.shape[-2:], mode='bilinear', align_corners=False)
        terms["loss_cal"] = mean_flat((res - cal_resized) ** 2)

        if coarse_pred is not None:
            # coarse v-target at 64x64
            res_small = F.interpolate(res, size=coarse_pred.shape[-2:], mode='bilinear', align_corners=False)
            noise_small = F.interpolate(noise, size=coarse_pred.shape[-2:], mode='bilinear', align_corners=False)
            sqrt_a_s = _extract_into_tensor(self.sqrt_alphas_cumprod, t, res_small.shape)
            sqrt_1a_s = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, res_small.shape)
            v_target_small = compute_v_target(res_small, noise_small, sqrt_a_s, sqrt_1a_s)
            terms["loss_v_coarse"] = mean_flat((v_target_small - coarse_pred) ** 2)

            # cross-scale consistency
            terms["loss_cs"] = cross_scale_consistency_loss(coarse_pred, fine_out)

            # dice+bce on low-noise steps
            low_noise_mask = (t < self.num_timesteps // 2).float().view(-1, 1, 1, 1)
            if low_noise_mask.sum() > 0:
                terms["loss_dice"] = (dice_bce_loss(fine_out, res) * low_noise_mask.squeeze()).mean()
            else:
                terms["loss_dice"] = th.tensor(0.0, device=x_start.device)

            terms["loss"] = (terms["loss_v_fine"] + terms["loss_v_coarse"]
                             + lambda_dice * terms["loss_dice"]
                             + lambda_cross_scale * terms["loss_cs"])
        else:
            terms["loss"] = terms["loss_v_fine"]

        return (terms, fine_out)

    def training_losses_segmentation_hybrid(self, model, x_start, t, model_kwargs=None, noise=None, lambda_dice=0.1):
        """
        Hybrid v2 training loss: v-prediction + calibration + dice (low-noise).
        model 返回 (out, cal) 格式（与 v1 相同，无 coarse 阶段）。
        """
        from .wmh_losses import compute_v_target, dice_bce_loss
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start[:, -1:, ...])
        ...
        model_out, cal = model(x_t, self._scale_timesteps(t), **model_kwargs)

        terms = {}
        terms["loss_v"] = mean_flat((v_target - model_out) ** 2)
        cal_resized = F.interpolate(cal, size=res.shape[-2:], mode='bilinear', align_corners=False)
        terms["loss_cal"] = mean_flat((res - cal_resized) ** 2)

        low_noise_mask = (t < self.num_timesteps // 2).float().view(-1, 1, 1, 1)
        if low_noise_mask.sum() > 0:
            terms["loss_dice"] = (dice_bce_loss(model_out, res) * low_noise_mask.squeeze()).mean()
        else:
            terms["loss_dice"] = th.tensor(0.0, device=x_start.device)

        terms["loss"] = terms["loss_v"] + lambda_dice * terms["loss_dice"]
        return (terms, model_out)

    def p_sample_loop_known_hybrid(self, model, shape, img, step=1000, clip_denoised=True, model_kwargs=None, device=None):
        """Hybrid v2 单阶段采样：v-prediction + highway encoder。"""
        if device is None:
            device = next(model.parameters()).device
        img = img.to(device)
        noise = th.randn_like(img[:, :1, ...]).to(device)
        ...
        if self.dpm_solver:
            ...
            _last_cal = [None]
            def _hybrid_unwrap(x, t, **kwargs):
                out, cal_out = model(x, t, **kwargs)
                _last_cal[0] = cal_out.detach()
                return out

            model_fn = model_wrapper(_hybrid_unwrap, noise_schedule, model_type="v", model_kwargs=model_kwargs)
            dpm_solver = DPM_Solver(model_fn, noise_schedule, algorithm_type="dpmsolver++",
                                    correcting_x0_fn="dynamic_thresholding", img=img[:, :-1, ...])
            sample, _ = dpm_solver.sample(
                noise.to(dtype=th.float), steps=step, order=2,
                skip_type="time_uniform", method="multistep",
            )
            sample = sample.detach()
            cal = _last_cal[0] if _last_cal[0] is not None else th.zeros_like(sample[:, :1, ...])
            ...
            for i in indices:
                t = th.tensor([i] * shape[0], device=device)
                if img_iter.shape[1] != x_noisy.shape[1]:
                    img_iter = torch.cat((org_MRI, img_iter), dim=1)
                with th.no_grad():
                    out = self.p_sample(model, img_iter.float(), t,
                                        clip_denoised=clip_denoised, model_kwargs=model_kwargs)
                img_iter = out["sample"]
            sample = img_iter
            cal = out.get("cal", th.zeros_like(sample[:, :1, ...]))

        from .utils import norm
        cal_resized = F.interpolate(cal, size=sample.shape[-2:], mode='bilinear', align_corners=False)
        cal_out = torch.clamp(cal_resized + 0.25 * sample[:, -1:, :, :], 0, 1)
        x_noisy = noise if self.dpm_solver else x_noisy
        return sample, x_noisy, img, cal, cal_out


    def p_sample_loop_known_wmh(self, model, shape, img, step=1000, clip_denoised=True, model_kwargs=None, device=None):
        """Two-stage WMH sampling: coarse (64x64) then fine (256x256)."""
        if device is None:
            device = next(model.parameters()).device
        img = img.to(device)
        noise = th.randn_like(img[:, :1, ...]).to(device)
        ...
        if self.dpm_solver:
            ...
            # Wrap model: unwrap WMH tuple -> fine_out tensor, cache cal
            _last_cal = [None]
            def _wmh_unwrap(x, t, **kwargs):
                out = model(x, t, **kwargs)
                if isinstance(out, tuple):
                    (fine_out, cal_out), _ = out
                    _last_cal[0] = cal_out.detach()
                    return fine_out
                return out

            model_fn = model_wrapper(_wmh_unwrap, noise_schedule, model_type="v", model_kwargs=model_kwargs)
            dpm_solver = DPM_Solver(model_fn, noise_schedule, algorithm_type="dpmsolver++",
                                    correcting_x0_fn="dynamic_thresholding", img=img[:, :-1, ...])
            sample, _ = dpm_solver.sample(
                noise.to(dtype=th.float), steps=step, order=2,
        ...
        cal_resized = F.interpolate(cal, size=sample.shape[-2:], mode='bilinear', align_corners=False)
        cal_out = torch.clamp(cal_resized + 0.25 * sample[:, -1:, :, :], 0, 1)
        return sample, x_noisy if not self.dpm_solver else noise, img, cal, cal_out
