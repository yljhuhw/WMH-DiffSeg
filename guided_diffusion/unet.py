...
class UNetModel_hybrid(nn.Module):
    """
    Hybrid v2: v1 highway encoder + WaveletModulation skip connections + v-prediction.
    去掉了 WMH 的 HybridEncoder 和 CoarseDiffusionHead，保留小波调制和 v-prediction。
    输入: [B, 4, H, W] (image 3ch + noisy_mask 1ch)
    输出: (out [B,1,H,W], cal [B,1,H,W])
    """
    ...
    def __init__(
        ...
        # WaveletModulation: 保存 skip 通道数副本（output_blocks 弹出顺序与 input_block_chans 倒序一致）
        input_block_chans_copy = list(input_block_chans)
        self.wavelet_modules = nn.ModuleList(
            [WaveletModulation(ich) for ich in reversed(input_block_chans_copy)]
        )
    ...
    def forward(self, x, timesteps, y=None):
        ...
        h = x.type(self.dtype)
        c = h[:, :-1, ...]  # image channels only (no noisy mask)
        anch, cal = self.hwm(c)
        ...
        for ind, module in enumerate(self.input_blocks):
            ...
            if ind == 0:
                h = module(h, emb)
                h = h + th.cat((anch[0], anch[0], anch[1]), 1).detach()  # highway injection
            else:
                h = module(h, emb)
            hs.append(h)
        ...
        for i, module in enumerate(self.output_blocks):
            skip = hs.pop()
            skip = self.wavelet_modules[i](skip)  # wavelet modulation on skip
